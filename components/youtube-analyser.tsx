...
export default function YouTubeAnalyser() {
  ...
  async function handleFileUpload(file: File) {
    ...
    if (!file.type.startsWith('video/') && !file.name.toLowerCase().match(/\.(mp4|mov|m4v|avi|mkv)$/)) {
      setStatus('error')
    ...
    try {
      const createResponse = await fetch('/api/uploads/create-url', {
        method: 'POST',
    ...
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-200/70">
      ...
      <div className="mt-6 space-y-4">
        <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4">
          ...
          <label className="mt-4 flex cursor-pointer items-center justify-center rounded-2xl border border-dashed border-emerald-300 bg-white px-4 py-5 text-sm font-bold text-emerald-700 transition hover:bg-emerald-50">
            Choose video file
        ...
        <p className="text-center text-xs text-slate-500">Required: video/link, teams, colours, coached team and structured goals/points score.</p>
      </div>
